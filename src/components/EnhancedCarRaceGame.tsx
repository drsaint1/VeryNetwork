...
const EnhancedCarRaceGame: React.FC<EnhancedCarRaceGameProps> = ({
  ...
  const claimFastTokens = useCallback(async () => {
    ...
    try {
      ...
      refetchTokenBalance();
      showPopup("🪙 FAST tokens claimed successfully!");
    } catch (error) {
  ...
  useEffect(() => {
    if (isTxConfirmed && currentTxHash) {
      ...
      if (autoSubmitResults) {
        showPopup("✅ Results auto-saved on Very!");
      } else {
  ...
  const submitRaceResult = useCallback(async () => {
    ...
    try {
      ...
      if (autoSubmitResults) {
        showPopup("⏳ Auto-saving to Very blockchain...");
      } else {
        showPopup("⏳ Submitting to Very blockchain...");
      }
    } catch (error) {
      setSubmissionStatus("error");
      setIsSubmittingScore(false);
  ...
  const animate = useCallback(() => {
    ...
    if (
      ...
      if (currentTime - animationStartTime.current > maxRetryTime) {
        console.log("⚠️ Refs unavailable too long, stopping animation loop");
    ...
    upcomingCarsRef.current.forEach((upcomingCar) => {
      ...
      if (!upcomingCar.userData.initialized) {
        ...
        console.log(`🚗 ${upcomingCar.userData.vehicleType || 'unknown'} vehicle created with speed: ${upcomingCar.userData.forwardSpeed.toFixed(2)}`);
      }
      ...
      >
        ...
        >
          ...
          >
            ...
            >
              💡 <strong>Tip:</strong> After minting your Bike, you can
            ...
            >
              ...
              >
                <div style={{ color: "#ffd700" }}>
                  💡 Earn FAST tokens by racing:
                </div>
                <div style={{ color: "#ffffff", marginTop: "2px" }}>
                  • Submit race results to earn FAST tokens • Higher scores = more
                  FAST tokens • Claim when ready
                </div>
                <div
                  style={{
                    color: "#22c55e",
                ...
                >
                  ✅ One transaction: Submit race + earn FAST tokens
                </div>
              </div>
            </div>
          )}
