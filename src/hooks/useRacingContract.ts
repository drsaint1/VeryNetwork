...
export const RACING_ABI = [
  ...
  },
  {
    ...
    name: "submitRaceResultWithFastTokens",
    outputs: [],
    stateMutability: "nonpayable",
    type: "function",
  },
    ...
    outputs: [
      {
        ...
        internalType: "struct VeryRacing.RaceCar",
        name: "",
        type: "tuple",
      },
    ],
    ...
    outputs: [
      {
        ...
        internalType: "struct VeryRacing.RaceResult[]",
        name: "",
        type: "tuple[]",
      },
    ],
...
export const useRacingContract = () => {
  ...
  const mintStarterCar = useCallback(async () => {
    try {
      ...
      const errorMessage = err.message || "Failed to mint starter vehicle";
  ...
  const mintBike = useCallback(async () => {
    try {
      setError(null);
      const txHash = await writeContractAsync({
        address: RACING_CONTRACT_ADDRESS,
      ...
      const errorMessage = err.message || "Failed to mint bike";
      setError(errorMessage);
      throw err;
    }
  }, [writeContractAsync]);

  const mintTruck = useCallback(async () => {
    try {
      setError(null);
      const txHash = await writeContractAsync({
        address: RACING_CONTRACT_ADDRESS,
      ...
      const errorMessage = err.message || "Failed to mint truck";
      setError(errorMessage);
      throw err;
    }
  }, [writeContractAsync]);
  ...
  const getCarColorName = (carName: string): string => {
    ...
    if (name.includes("starter") || name.includes("car")) return "Blue";
    if (name.includes("bike")) return "Red";
    if (name.includes("truck")) return "Green";
