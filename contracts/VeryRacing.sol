...
contract VeryRacing is ERC721URIStorage, Ownable, ReentrancyGuard, Pausable {
    struct RaceCar {
        uint256 speed;
        uint256 handling;
    ...
    uint256 public constant BIKE_COST = 0.01 ether;
    uint256 public constant CAR_COST = 0.05 ether;
    uint256 public constant TRUCK_COST = 0.08 ether;
    ...
    constructor() ERC721("Very Racing Cars", "VRC") Ownable(msg.sender) {
    ...
    function mintBike() external payable whenNotPaused {
        require(msg.value >= BIKE_COST, "Insufficient payment");
    ...
    function mintCar() external payable whenNotPaused {
        require(msg.value >= CAR_COST, "Insufficient payment");
    ...
    function mintTruck() external payable whenNotPaused {
        require(msg.value >= TRUCK_COST, "Insufficient payment");
    ...
    function mintPremiumCar() external payable whenNotPaused {
        ...
        if (seed % 2 == 0) {
            ...
            require(msg.value >= TRUCK_COST, "Insufficient payment for Truck upgrade");
    ...
    function submitRaceResult(
        ...
        _submitRaceResult(player, carId, score, distance, carsAvoided, bonusCollected, tournamentId, false);
    }
    
    function submitRaceResultWithFastTokens(
        address player,
        uint256 carId,
        uint256 score,
        uint256 distance,
        ...
        _submitRaceResult(player, carId, score, distance, carsAvoided, bonusCollected, tournamentId, mintTokens);
    }
    ...
    function claimFastTokens(address player) external whenNotPaused {
    ...
    function _generateTokenURI(RaceCar memory car, uint256 tokenId) internal pure returns (string memory) {
        ...
        return string(abi.encodePacked("https://api.veryracing.com/metadata/", _toString(tokenId)));
    }
