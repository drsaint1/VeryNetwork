...
async function main() {
  console.log(" Deploying All Contracts to Very Mainnet\n");
  ...
  try {
    ...
    console.log(" Deploying VeryRacing contract...");
    const VeryRacing = await ethers.getContractFactory("VeryRacing");

    const racingContract = await VeryRacing.deploy();
    await racingContract.waitForDeployment();

    const racingAddress = await racingContract.getAddress();
    console.log("✅ VeryRacing deployed to:", racingAddress);

    console.log("\n Deploying VeryTournaments contract...");
    const VeryTournaments = await ethers.getContractFactory("VeryTournaments");

    const tournamentsContract = await VeryTournaments.deploy(racingAddress);
    await tournamentsContract.waitForDeployment();

    const tournamentsAddress = await tournamentsContract.getAddress();
    console.log("✅ VeryTournaments deployed to:", tournamentsAddress);
    ...
    console.log(" VeryRacing Contract:", racingAddress);
    console.log(" VeryTournaments Contract:", tournamentsAddress);
    ...
    console.log("   • Race result submission with FAST token rewards");
...
main()
  .then(() => {
    console.log("\n ALL CONTRACTS DEPLOYED TO VERY MAINNET SUCCESSFULLY! ");
